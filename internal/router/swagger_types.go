...
type EvaluateRequest struct {
	// SQL rule text. Optional: defaults to the built-in flagship rule when empty.
	Rule string `json:"rule" example:"age BETWEEN 25 AND 40 AND province IN ('广东','江苏','浙江') AND active_score >= 85"`
	// Optional: explain an already-loaded rule by its ID (used only when "rule" is empty).
	RuleID int64 `json:"rule_id" example:"0"`
	// The wide-table user row to evaluate.
...
// Ping2Response is returned by GET /ping2.
type Ping2Response struct {
	Ping string `json:"ping" example:"pong2"`
}
