...
import (
	...
	"tcg-rulex-engine/internal/topics"
...
// InitTimeUsageConsumer 初始化 Kafka 消费者实例并订阅 PLAYER_BEAT_TIMELIMIT_USAGE。
//
//   - 初始化配置信息：复用 InitConsumer() 从 [kafka.consumer] 构建单例 *Consumer
//     （brokers / client_id / session / fetch 等全局配置）。
//   - 共享消费组：使用 SubscribeTopicSharedGroup，所有实例以 toml 配置的 group_id 原样加入
//     同一个 Kafka consumer group（不拼主机名）。多实例部署时由 Kafka 在实例间均衡分区，
//     每条消息只被一个实例消费——避免 N 台机器各自成组、重复累加 N 次。
//   - 重启续传：consumer group + 手动批量提交（DisableAutoCommit + CommitRecords），服务重启后
//     自动从上一次已提交的 offset 继续消费（group.id 稳定，不随主机名变化）；全新 group 才按
//     auto_offset_reset（本 topic 配置为 latest）起始。
//   - 并发：worker 数量取 per-topic max_workers（toml 配置为 32），启动时再按实际分区数收敛。
//
// 返回的 *Consumer 由调用方持有，用于优雅停机时 Close()。
// brokers 未配置时 InitConsumer 返回 nil，本函数返回错误。
//
// 消息体类型见 model.PlayerBeatTimeUsage（解码与归一化由消费服务层完成）。
func (k *Config) InitTimeUsageConsumer(handler func(record *kgo.Record)) (*Consumer, error) {
	consumer := k.InitConsumer()
	if consumer == nil {
		return nil, errors.New(
			"kafka consumer not initialized: no brokers configured under [kafka.consumer]",
		)
	}
	if err := consumer.SubscribeTopicSharedGroup(topics.TopicPlayerBeatTimeLimitUsage, handler); err != nil {
		return nil, fmt.Errorf("subscribe topic %s failed: %w", topics.TopicPlayerBeatTimeLimitUsage, err)
	}
	return consumer, nil
}
