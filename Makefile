# ══════════════════════════════════════════════════════════════════════════════
# Makefile — Ai-Rule-Engine For AIRuleX
#
# Usage:
#   make              → Show this help
...
optimize: ## Fix struct alignment, format code, and update deprecated APIs
	@# -test=false: fieldalignment -fix reorders struct FIELDS but not unkeyed
	@# composite LITERALS, which silently breaks compilation of table-driven
	@# tests (pkg/vm/*_test.go use positional {rule, row, want} literals).
	@# Test structs are about readability, not memory layout — skip them.
